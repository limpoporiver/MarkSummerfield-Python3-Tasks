...
def main():
    if len(sys.argv) == 1 or sys.argv[1] in {"-h", "--help"}:
        print(f"usage: {sys.argv[0]} file1 [file2 [... fileN]]".format(sys.argv[0]))
        sys.exit()
...
def print_users(users):
    ...
    print("{0:<{nw}} {1:^7} {2:{uw}}".format("Name", "ID", "Username", nw=namewidth, uw=usernamewidth))
    print("{0:-<{nw}} {0:-<7} {0:-<{uw}}".format("", nw=namewidth, uw=usernamewidth))
    ...
    for key in sorted(users):
        ...
        print("{0:.<{nw}} ({1.id:5}) {1.username:{uw}}".format(
